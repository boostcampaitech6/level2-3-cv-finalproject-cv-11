...
# Initialize a global cache for compiled functions
compiled_functions_cache = {}

def cupy_launch(strFunction, strKernel):
    # Create a unique key for the function and kernel combination
    cache_key = (strFunction, strKernel)
    
    # Check if the function is already compiled and cached
    if cache_key in compiled_functions_cache:
        return compiled_functions_cache[cache_key]
    
    # If not cached, compile the kernel and cache it
    compiled_function = cupy.cuda.compile_with_cache(strKernel).get_function(strFunction)
    compiled_functions_cache[cache_key] = compiled_function
