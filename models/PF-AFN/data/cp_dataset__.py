...
class CPDataset(data.Dataset):
    ...
    def __init__(self, dataroot, image_size=512, mode='test', semantic_nc=13, unpaired=False):
        ...
        self.data_path = osp.join(dataroot, 'test')
        ...
        # for im_name in redis.get(f'task{model_id}:model').decode('utf-8'):
        model_id = '00055_00.jpg'
        im_names.append(model_id)
        im_names.append('00055_00.jpg')
        c_names.append('00006_00.jpg')
        c_names.append('00006_00.jpg')
        
        # c_img = Image.open(io.BytesIO(redis.get(f'task:{real_id}:result')))
        # img = remove(c_img)
        # cm_img = Image.new('RGB', img.size, (255,255,255))
        ...
        #         if alpha == 0:
        #             cm_img.putpixel((x, y), (0,0,0))
        #         else:
        #             cm_img.putpixel((x, y), (255,255,255))
        
        # transform = Grayscale(num_output_channels=1)
        # cm_img = transform(cm_img).convert("RGB")
    ...
    def __getitem__(self, index):
        ...
        key = ['unpaired'] if self.unpaired else ['paired']
        c_name[key] = self.c_names
